...
CREATE INDEX IF NOT EXISTS collaboration_sessions_resource_idx ON collaboration_sessions(resource_id, resource_type);
CREATE INDEX IF NOT EXISTS collaboration_sessions_expires_idx ON collaboration_sessions(expires_at);
...
CREATE INDEX IF NOT EXISTS comments_resource_idx ON comments(entity_id, entity_type);
...
CREATE INDEX IF NOT EXISTS comments_resolved_idx ON comments(is_resolved);
...
-- 5. LIVE EDITS
CREATE TABLE IF NOT EXISTS live_edits (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS live_edits_session_idx ON live_edits(session_id, timestamp);
CREATE INDEX IF NOT EXISTS live_edits_resource_idx ON live_edits(resource_id, resource_type);
CREATE INDEX IF NOT EXISTS live_edits_applied_idx ON live_edits(applied);

-- 6. TRIGGERS
CREATE OR REPLACE TRIGGER collaboration_sessions_updated_at 
